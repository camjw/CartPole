...
class ModelHolder:
    ...
    def __init__(
        self, action_size, observation_size, batch_size, hidden_size=256, keep_prob=0.9
    ):
    ...
    def define_model(self):
        ...
        self.states = tf.placeholder(
            shape=[None, self.observation_size], dtype=tf.float32
        )
        self.q_s_a = tf.placeholder(shape=[None, self.action_size], dtype=tf.float32)
        ...
        l_1_weights = tf.Variable(
            tf.truncated_normal([self.observation_size, self.hidden_size])
        )
        l_2_weights = tf.Variable(
            tf.truncated_normal([self.hidden_size, self.action_size])
        )
        ...
        fully_connected_1 = tf.nn.tanh(tf.matmul(self.states, l_1_weights) + l_1_biases)
    ...
    def predict_batch(self, states, sess):
        return sess.run(
            self.logits, feed_dict={self.states: states, self.dropout: self.keep_prob}
        )
