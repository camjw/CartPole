...
class ModelHolder:
    ...
    def define_model(self):
        ...
        self._states = tf.placeholder(shape=[None, self._observation_size], dtype=tf.float32)
        self._q_s_a = tf.placeholder(shape=[None, self._action_size], dtype=tf.float32)
        ...
        l_1_weights = tf.Variable(tf.truncated_normal([self._observation_size, self._hidden_size]))
        l_2_weights = tf.Variable(tf.truncated_normal([self._hidden_size, self._hidden_size]))
        l_3_weights = tf.Variable(tf.truncated_normal([self._hidden_size, self._action_size]))
        l_1_biases = tf.Variable(tf.zeros([self._hidden_size]))
        l_2_biases = tf.Variable(tf.zeros([self._hidden_size]))
        l_3_biases = tf.Variable(tf.zeros([self._action_size]))

        #self._saver = tf.train.Saver([l_1_weights, l_2_weights, l_3_weights, l_1_biases,])
        fully_connected_1 = tf.nn.relu(tf.matmul(self._states, l_1_weights) + l_1_biases)
        fully_connected_2 = tf.nn.relu(tf.matmul(fully_connected_1, l_2_weights) + l_2_biases)
